...
<form name="formEntry" action="../sail/promotion.do" method="post"><input
	type="hidden" name="method" value="rptQueryPromotion"><input
	type="hidden" value="1" name="load">
<input type="hidden" value="${selectMode}" name="selectMode"> 

<p:navigation
	height="22">
	<td width="550" class="navigation">促销规则管理</td>
	<td width="85"></td>
</p:navigation> <br>
...
<p:body width="100%">
	<p:subBody width="90%">
		<table width="100%" align="center" cellspacing='1' class="table0"
			id="result">
			<tr class="content1">
				<td width="15%" align="center">促销名称</td>
				<td align="center"><input type="text" name="name" onkeypress="press()"
					value="${name}"></td>
				<td width="15%" align="center">活动ID</td>
	...
	<p:title>
		<td class="caption"><strong>促销规则列表：</strong></td>
	</p:title>
		...
		<table width="100%" align="center" cellspacing='1' class="table0"
			...
			<c:forEach items="${beanList}" var="item" varStatus="vs">
				<tr class="${vs.index % 2 == 0 ? 'content1' : 'content2'}">
					...
					${vs.index == 0 ? 'checked' : ''}
					pname="${item.name}" paddup="${item.isAddUp}" pdate1="${item.beginDate}"  pdate2="${item.endDate}" value="${item.id}"/></td>
					<td align="center" onclick="hrefAndSelect(this)">${item.name}</td>
					<td align="center" onclick="hrefAndSelect(this)">${my:get('isAddUp', item.isAddUp)}</td>
					<td align="center" onclick="hrefAndSelect(this)">${my:get('rebateType', item.rebateType)}</td>
					<td align="center" onclick="hrefAndSelect(this)">${item.beginDate}</td>
					<td align="center" onclick="hrefAndSelect(this)">${item.endDate}</td>
				</tr>
			</c:forEach>
		</table>
