...
(function($){
    ...
    $.addFlex = function(t, p)
    {
        ...
        p = $.extend({
             ...
             url: false, //ajax url
             method: 'POST', // data sending method POST must use UTF-8
             ...
             onSuccess: false,
             rwidth: 0,
             errormsg: Res._Guid['ERRORMSG'],
             nomsg: Res._Guid['NOMSG'],
             pagestat: Res._Guid['PAGESTAT'],
             procmsg: Res._Guid['PROCMSG'],
             bname: {add: Res._Public['ADD'], 
                     del: Res._Public['DEL'], 
                     update: Res._Public['UPDATE'], 
                     search: Res._Public['SEARCH']},
             resource: {refurbish: Res._Guid['REFURBISH'], 
                        oprColumns: '',//Res._Guid['OPRCOLUMNS'], 
                        page: Res._Guid['PAGE'], 
                        of: Res._Guid['OF'], 
                        oprTable: Res._Guid['OPRTABLE']},
          ...
          {
              if (p.cacheEle == null)
              p.cacheEle = document.getElementById('cacheEle'); 
              
              if (p.cacheFlag == null)
              p.cacheFlag = document.getElementById('cacheFlag');
          }
        ...
        var g = {
            ...
            dragStart: function (dragtype,e,obj) { //default drag function start
                ...
                if (dragtype=='colresize') //column resize
                ...
                else if (dragtype=='colMove') //column header drag
                    {
                        ...
                        eval("$(this.colCopy).css({position:'absolute',float:'left',display:'none', textAlign: obj.align});");
            ...
            showError : function (data)
            {
                ...
                $('.pPageStat',this.pDiv).html('<font color=red>' + errorStr + '</font>'); 
                return false;
            },
            addData: function (data) { //parse data
                ...
                // TODO zhuzhu
                if (p.cache == 0 && window.JSON)
                {    
                    p.cacheFlag.value = 1;
                    
                    p.cacheEle.value = JSON.stringify(data);
                    
                    //alert(JSON.parse(p.cacheEle.value).total);
                }
                
                var flexi_Checks = document.getElementById('flexi_Check');
                ...
                setSelect(document.getElementById('rp'), p.rp);
                                ...
                                function ()
                                    {
                                        ...
                                        var td = document.createElement('td');
                                         ...
                                         if (p.extAtt)
                                         {
                                             var extMap = p.extAtt[iname];
                                             
                                             if (extMap)
                                             {
                                                 bs = parserString(extMap['begin'], row.cell);  
                                                 es = parserString(extMap['end'], row.cell);    
                                             }
                                         }
                                         td.align = this.align;
                                         ...
                                         else
                                         {
                                            ...
                                            var toFixed = $(this).attr('toFixed');
                                            
                                            if (ccs && p.def)
                                            {
                                                var listmap = p.def[ccs];
                                                
                                                if (listmap)
                                                for (var ii = 0; ii < listmap.length; ii++)
                                                {
                                                    var mapInner = listmap[ii];
                                                    if (mapInner.key == con)
                                                    {
                                                        con = mapInner.value;
                                                        break;
                                            ...
                                            {
                                                con = parseFloat(con).toFixed(toFixed) + '';
                                            }
            ...
            changeSort: function(th) { //change sortorder
                ...
                var abbr = $(th).attr('abbr2') ? $(th).attr('abbr2') : $(th).attr('abbr');
            ...
            populate: function (flush) { //get latest data
                ...
                var param = [
                     { name : 'nowPage', value : p.newp }
                    ,{ name : 'pageSize', value : p.rp }
                    ,{ name : 'JSON', value : 'JSON' }
                    ,{ name : 'sortname', value : p.sortname}
                    ,{ name : 'sortorder', value : p.sortorder }
                    ,{ name : 'query', value : p.query}
                    ,{ name : 'queryMode', value : p.queryMode}
                    ,{ name : 'oprAction', value : p.oprAction}
                    ,{ name : 'qtype', value : p.qtype}
                ];     
                
                if (p.queryCondition)
                {
                    for(var att in p.queryCondition)
                    {
                        var mmaps = {};
                ...
                //cache data    
                //alert(p.cacheEle);
                //alert(p.cacheEle.value);
                if (!flush && window.JSON && p.cache == 0 && p.cacheEle && p.cacheEle.value && p.cacheFlag && p.cacheFlag.value == 1 && p.oprAction == 0)
                {
                    var data = JSON.parse(p.cacheEle.value);
        ...
        if (p.colModel)
        {
            ...
            if (p.width == 'auto')
            {
                pageWidth = document.body.offsetWidth - (p.colModel.length - 2) * 13 + (p.colModel.length - 3) * 1 - p.rwidth - 25;
            }
            else
            {
                pageWidth = p.width - (p.colModel.length - 2) * 13 + (p.colModel.length - 3) * 1 - 25;
            }
            ...
            for (i=0;i<p.colModel.length;i++)
            {
                ...
                if (cm.width)
                { 
                    if ((cm.width + "").indexOf("%") != -1)
                    {
                        var wh = parseInt(cm.width.substring(0, cm.width.length - 1));
                        
                        widthArray[i] = Math.ceil(pageWidth * wh /100) - 1;
                    }
                    else if (cm.width == 'auto')
                    {
                        autoIndex = i; 
                    }
                    else if (cm.width == 'auto2')
                    {
                        autoIndex = i; 
                        auto2 = 1; 
                    }
                    ...
                    {
                        widthArray[i] = cm.width;
                    }
                }
                
                totalWidth += parseInt(widthArray[i] + '');
            }
            
            if (autoIndex != -1)
            {
                var autoWidth = pageWidth - totalWidth;
                
                //alert(pageWidth);
                //alert(totalWidth);
                //alert(autoWidth + totalWidth);
                ...
                if (auto2 == 0)
                widthArray[autoIndex] = autoWidth;
                else
                // scrol is 18px
                widthArray[autoIndex] = autoWidth - 18;
            }
            ...
            for (i=0;i<p.colModel.length;i++)
                {
                    ...
                    //if (cm.name)
                    if (cm.cname && cm.sortable)
                        $(th).attr('abbr2', cm.cname);
                    ...
                    //th.idx = i;
                    $(th).attr('axis','col'+i);
                    $(th).attr('content', cm.content);
                    
                    if (cm.cc)
                    $(th).attr('cc', cm.cc);
                    
                    if (cm.toFixed)
                    $(th).attr('toFixed', cm.toFixed);
        ...
        g.gDiv.style.width = (document.body.offsetWidth - p.rwidth) + 'px';
            ...
            for (i=0;i<p.buttons.length;i++)
                {
                    ...
                    //0503,0504
                    if ((btn.auth && p.auth) || (btn.auth == false && p.auth) || (btn.auth == 'false' && p.auth))
                    {
                        var hasAuth = false;
                        ...
                        {
                            var aauths = btn.auth.split(',');
                            
                            for(var m = 0; m < p.auth.length; m++)
                            {
                                for(var n = 0; n < aauths.length; n++)
                                {
                                    if (aauths[n] == p.auth[m])
                                    {
                                        hasAuth = true;
                                        break;
                                    }
                    ...
                    if (!btn.separator)
                    {
                        if (dis)
                        {
                            ...
                            if (btn.caption)
                            btnDiv.innerHTML = "<div id=b_"+btn.id+"><span id=s_"+btn.id+">"+ btn.caption +"</span></div>";
                            else
                            btnDiv.innerHTML = "<div id=b_"+btn.id+"><span id=s_"+btn.id+">"+ p.bname[btn.id.toLowerCase()] +"</span></div>";
        ...
        $(g.bDiv).after(g.pDiv);
        var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">'+p.resource['page']+' <input type="text" size="4" value="1" /> '+p.resource['of']+' <span> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton" title="' + p.resource['refurbish'] + '"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageStat"></span></div>';
        $('div',g.pDiv).html(html);
        
        $('.pReload',g.pDiv).click(function(){p.oprAction = 3;g.populate()});
        ...
        if ($.browser.msie&&$.browser.version<7) $('.pButton',g.pDiv).hover(function(){$(this).addClass('pBtnOver');},function(){$(this).removeClass('pBtnOver');});
            ...
            $('.pDiv2',g.pDiv).prepend("<div class='pGroup'><select name='rp' id='rp'>"+opt+"</select></div> <div class='btnseparator'></div>");
        ...
        if (p.title)
        {
            ...
            if (p.showTableToggleBtn)
                {
                    $(g.mDiv).append('<div class="ptogtitle" title="'+p.resource['oprTable']+'"><span></span></div>');
        ...
        if ($('th',g.hDiv).length)
        {
            ...
            .html('<div></div>')
            .attr('title',p.resource['oprColumns'])
            .click
            (
                function ()
                {
        ...
        return this.each( function() 
        {
                if (!docloaded)
                {
                    ...
                    (
                        function ()
                        {
                            gobal_guid = $.addFlex(t, p);
                        }
                    );
                    
                    //alert(thisObj.populate);
                } 
                else 
                {
                    gobal_guid = $.addFlex(this, p);
                }
            });
...
function parserString(orgStr, map)
{
    //aa{id}dd --> aa123dd
    // aa\{id\}dd -->aa{id}dd
    var ps = orgStr;
...
function getSplit(orgStr, map)
{
    var index1 = orgStr.indexOf("{");
    ...
    if (index2 < orgStr.length - 1)
    {
        var lastStr = orgStr.substring(index2 + 1);
    }
    
    var key = orgStr.substring(index1 + 1, index2);
...
function checkAll(obj)
{
    ...
    var tem = document.getElementsByTagName("input");
    
    for (var i = 0; i < tem.length; i++)
    {
        if (tem[i].type.toLowerCase() == 'checkbox' && tem[i].name == 'checkb')
        {
            tem[i].checked = value;
        }
    }
...
/**
 * JS非常奇怪，靠js画出来的的对象直接操作有问题，必须靠getElementsByTagName
 */
