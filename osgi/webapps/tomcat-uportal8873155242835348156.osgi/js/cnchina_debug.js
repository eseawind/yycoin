 ...
 * sp 分隔符
 */
function getSpell(str, single, ignoreLetter, sp){
    /*********qiushuiwuhen(2002-9-16)********/
    var i,c,t,p,ret="";
    if(sp==null)sp="";
    for(i=0;i<str.length;i++){
        if(str.charCodeAt(i)>=0x4e00)
        {
            ...
            if(p>-1&&p<3755){
                ...
                if(t>0)ret+=getSingle(qswhSpell[t-1], single)+sp;
            }
        }
        else
        {
...
function trans(){
    result.value="结果如下："
    result.value+='\nUrlEncode(source.value):\n'+UrlEncode(source.value)
    result.value+='\n\ngetSpell(source.value):\n'+getSpell(source.value)
    result.value+='\n\ngetSpell(source.value,"\'"):\n'+getSpell(source.value,"'")
}
...
function spellList()
{
	  ...
	  changeFun = targetObject.onchange;

       with(getEvent()){
          with(targetObject){
              if(keyCode < 48 && keyCode != 39 && keyCode != 37)return;

              if ((keyCode == 39 || keyCode == 37) && oldSel != '')
              {
              	 // 左右自动匹配查找上下
	              if (!window.event)
	              {
	              	  // 由于firefox里面的左右被占用，所以是 altKey + 左右
	              	  if (!getEvent().altKey)
	              	  {
	              	  	  returnValue = false;
	              	      return;
				 ...
				 var once = true;
              	 ...
              	 if (upInner)
              	 {
              	 	 ...
              	 	 if (begin < 0)
              	 	 {
					     begin = options.length - 1;
              	 	 }

					 for(i = begin; i >= 0; i--)
					 {
		                  if(!$Attribute(options[i], 'sp'))
		                  {
		                      var tmp="",arr=getSpell(options[i].text, true, false, "'").split("'");
	                      ...
	                      // 向上寻找
		                  if($Attribute(options[i], 'sp').indexOf(oldSel) == 0)
		                  {
		                  	  selectedIndex = i;
                  ...
                  else
                  {
                  	 var begin = pos + 1;
              	 	 ...
              	 	 if (begin > options.length - 1)
              	 	 {
					     begin = 0;
              	 	 }

                     for(i = begin; i < options.length; i++)
					 {
		                  if(!$Attribute(options[i], 'sp'))
		                  {
		                      var tmp="",arr=getSpell(options[i].text,true,false,"'").split("'");
              ...
              for(i=0;i<length;i++){
                  ...
                  if($Attribute(options[i], 'sp').indexOf(sel)==0)
                  {
                  	selectedIndex = i;
