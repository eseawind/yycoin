...
<input type="hidden" name="taskCode" value="${bean.taskCode }"> 
	...
	<p:subBody width="98%">
		...
		<p:table cells="2">
            ...
            <textarea name='partaker'
					head='参与人' id='partaker' onclick='selectpartaker(this)' rows=2 cols=80 style='cursor: pointer;' readonly=true>${bean.partaker}</textarea>
            <input
            ...
            <p:pro field="planFinishTime" cell="0">
            	时：<select name="finishTime" class="select_class" values="${bean.finishTime}" oncheck="notNone;isInt;range(0, 23)" style="width: 50px">
                            <p:option type="[0,23]"/>
                  </select>&nbsp;
            	分：<select name="finishMin" class="select_class" values="${bean.finishMin}" oncheck="notNone;isInt;range(0, 59)" style="width: 50px">
                            <p:option type="[0,59]"/>
                   </select>
            </p:pro>
