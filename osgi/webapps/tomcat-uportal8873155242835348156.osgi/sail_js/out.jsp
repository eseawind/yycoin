...
function check(isAdd)
{
    ...
    var noSailJiuBi = false;
    
    var commonproduct = false; 
    
    var nocommonproduct = false;
    ...
    {
    	if (proNames[i].oldgoods == 9522152)
    	{
    		sailJiuBi = true;
    		
    		proName = proNames[i].value ;
    	}
    	else
    	{
    		noSailJiuBi = true;
    	...
    	if (sailJiuBi && noSailJiuBi)
    	{
    		alert('旧货的产品必须单独销售(或者都是旧货)，不允许和其他的产品类型一起销售:' + proName);
    	...
    	{
    		nocommonproduct = true;
    	}
    
    	if (commonproduct && nocommonproduct)
    	{
    		alert('通用产品不允许和其他的产品一起销售:' + proName);
    ...
    for (var i = 1; i < taxrates.length; i++)
    {
        ...
        if (i > 1)
        {
        	if (taxrates[i].value != taxrates[i-1].value)
        	{
	        	alert('非新增库单时，一张单上只能有一种税率!');
	            taxrates[i].focus();
	            return false;
        	}
        }
		
		eachTaxRate = taxrates[i].value;
