...
function addBean1(param)
{
	...
	if ($$('type') == '1')
	{
        $.messager.prompt('对账单邮件', '请输入接收邮件地址', "", function(value, isOk){
            if (isOk)
            {
                ...
                if (re.test(value))
                {
                    $ajax('../customerService/feedback.do?method=previewFeedBackCheck&type=1&mailAddress='+value+'&taskId=${feedBack.id}', callBackFun1);
                }
                else
                {
                    alert('请确定邮件地址的格式');
	...
	{
		document.location.href = '../customerService/feedback.do?method=previewFeedBackCheck&type=0&taskId=${feedBack.id}';
	}
...
<form name="formEntry" action="../customerService/feedback.do" method="post">
	<input type="hidden" name="method" value="addFeedBackCheck">
...
<p:body width="100%">
	...
	<p:subBody width="98%">
		...
		<p:table cells="2">
			...
			<p:pro field="exceptionProcesserName" innerString="readonly=true" value="${bean.exceptionProcesserName}" cell="0">
			...
			<c:if test="${feedBack.type == 5}">
				<p:pro field="actualExceptionReason" value="${bean.actualExceptionReason}" innerString="readonly=readonly">
					<p:option type="229" empty="true"></p:option>
				</p:pro>
			
				<p:pro field="resolve" value="${bean.resolve}" innerString="readonly=readonly">
					<p:option type="230" empty="true"></p:option>
				</p:pro>
	
				<p:pro field="resolveText" cell="0" innerString="rows=2 cols=55 readonly=readonly" value="${bean.resolveText}"/>
			</c:if>
	...
	<p:button leftWidth="100%" rightWidth="0%">
		<div align="right"><input
            type="button" name="print" class="button_class"
            onclick="addBean1(0)"
            value="&nbsp;&nbsp;下载打印&nbsp;&nbsp;">&nbsp;&nbsp;<input
            type="button" name="email" class="button_class"
            onclick="addBean1(1)"
            value="&nbsp;&nbsp;邮件&nbsp;&nbsp;">&nbsp;&nbsp;<input type="button"
