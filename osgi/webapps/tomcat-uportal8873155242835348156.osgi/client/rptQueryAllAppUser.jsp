...
<form name="formEntry" action="../client/client.do" method="post"><input
	type="hidden" name="method" value="rptQueryAllAppUser">
		...
		<table width="100%" align="center" cellspacing='1' class="table0"
			...
			<tr class="content1">
				...
				<td align="center"><input type="text" name="name" onkeypress="press()"
					value="${name}"></td>
				<td width="15%" align="center">用户编码</td>
			...
			<c:forEach items="${list}" var="item" varStatus="vs">
				<tr class="${vs.index % 2 == 0 ? 'content1' : 'content2'}">
					<td align="center"><input type="radio" name="beans" 
						pname="${item.loginName}" value="${item.id}" ${vs.index== 0 ? "checked" : ""}/></td>
					
					<td align="center" onclick="hrefAndSelect(this)">${item.id}</td>
					<td align="center" onclick="hrefAndSelect(this)">${item.loginName}</td>
				</tr>
			</c:forEach>
		</table>
			
		<p:formTurning form="formEntry" method="rptQueryAllAppUser"></p:formTurning>
