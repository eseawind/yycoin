...
/**
 * 
 * 业务要求勾款审批时要对销售审批状态进行检查。
 * 1）勾款时检查销售单是否通过库管审批，如果通过，则产生预收/应收凭证，如果没有通过，则不产生凭证；                                            
 * 2）库管审批时，检查该单是否勾过款，如果勾过款，则生应收/主营业务收入等销售凭证和勾款预收/应收凭证，否则，只生成应收/主营业务收入等凭证；
 * 因此，在勾款审批，也即收款申请审批通过时，如果库管未审批，也产生凭证，但数据不存入t_center_finanace
 *
 * @author fangliwen 2012-6-7
 */
@Entity
@Table(name = "T_CENTER_MIDFINANCE")
public class FinanceMidBean implements Serializable
{
    @Id
    private String id = "";
    ...
    @Ignore
    private List<FinanceItemMidBean> itemList = null;

    /**
     * default constructor
     */
    public FinanceMidBean()
    {
    }
     ...
     * @return the itemList
     */
    public List<FinanceItemMidBean> getItemList()
    {
        return itemList;
    }
     ...
     *            the itemList to set
     */
    public void setItemList(List<FinanceItemMidBean> itemList)
    {
    ...
    public boolean equals(Object obj)
    {
        ...
        if ( ! (obj instanceof FinanceMidBean)) return false;
        final FinanceMidBean other = (FinanceMidBean)obj;
        if (id == null)
        {
