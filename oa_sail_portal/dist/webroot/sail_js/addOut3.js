...
function clears()
{
	...
	document.getElementById('sailType').value = '';
...
function getProductRelation(ox)
{
    ...
    if (indes == 0)
    {
        ...
        if (!setObj(oo, ox[0]))
        {
        	alert('选择的产品的销售类型和产品类型不一致:' + ox[0].pname);
        	return;
        }
        ...
        var showJSON = JSON.parse(ox[0].pshowjosn);
        
        reloadShow(os.cells[8].childNodes[0], showJSON);
    }
    ...
    for(var i = indes; i < ox.length; i++)
    {
        ...
        {
        	delLast()
        	alert('选择的产品的销售类型和产品类型不一致:' +  ox[i].pname);
            return;
        }
        ...
        var sels = newTr.getElementsByTagName('SELECT');
        
        var showJSON = JSON.parse(ox[i].pshowjosn);
        
        reloadShow(sels[1], showJSON);
    }
}
...
{
	var sailTypeVal = $$('sailType');
    var productTypeVal = $$('productType');
    
    if (sailTypeVal == '')
    {
        $O('sailType').value = dest.pproductsailtype;
        $O('productType').value = dest.pproducttype;
    }
    else
    {
        //验证产品的类型是否一致
        if (dest.pproducttype != productTypeVal || dest.pproductsailtype != sailTypeVal)
        {
            return false;
        }
    }
    ...
    src.productsailtype = dest.pproductsailtype;
