...
function ccs(obj)
{
	...
	//计算毛利与毛利率
	var cost = os.cells[4].childNodes[0].value;	
	...
	{
		os.cells[7].childNodes[0].value = formatNum(profit, 2);
		os.cells[8].childNodes[0].value = profitRatio + '%';
	}

	//计算税额
	var taxrate = os.cells[5].childNodes[0].value;
	var tax = 0;
	
	var mtype = os.cells[0].childNodes[0].mtype;
		
	var oldgoods = os.cells[0].childNodes[0].oldgoods;
	
	var inputrate = os.cells[0].childNodes[0].inputrate;
	...
	os.cells[6].childNodes[0].value = formatNum(mul(m, tax), 2);
...
function getProductRelation(ox)
{
    ...
    if (indes == 0)
    {
        ...
        os.cells[1].childNodes[0].title = '当前产品的最大数量:' + ox[0].pamount;
        os.cells[1].childNodes[0].oncheck = 'range(0, ' + ox[0].pamount + ')';
        
        os.cells[2].childNodes[0].value = ox[0].pinputprice;
        
        //显示价格
        os.cells[4].childNodes[0].value = ox[0].pprice;
        if (ox[0].pcode == '191175852' || ox[0].pcode == '191165735')
        	os.cells[4].childNodes[0].readOnly=false;
        else
        	os.cells[4].childNodes[0].readOnly=true;
        //实际成本
        os.cells[4].childNodes[1].value = ox[0].prealprice;
        
        if (ox[0].pmtype == '0')
    	{
        	os.cells[5].childNodes[0].value = '0';
    	}
        else
    	{
        	if (ox[0].poldgoods == '9522152')
        	{
        		os.cells[5].childNodes[0].value = '0.02';
        	}
        	else if (ox[0].poldgoods == '9522158')
        	{
        		os.cells[5].childNodes[0].value = '0.17';
        	}
        	else {
        		os.cells[5].childNodes[0].value = '0';
        	}
